...
class CommandControl:
    ...
    def create_prompt(self):
        print("\n")
        print("This is InsightsHub API command prompt.")
        print("Trigger any option from below menu for your MindMonnect MQTT Agent.")
        if self.clientId == "":
            self.clientId = input("\033[94m Enter MQTT Client Id : \033[0m")
        else:
            print(f'\033[94m Operating on Client Id : {self.clientId} \033[0m')

        print("\033[94m ------- \n 1. JetPump On \n 2. JetPump Off \n 3. Check Command Status "
              "\n 4. Check Command Response \n 5. Trigger Download Model "
              "\n 6. Create Custom Topic and Custom Data Mapping \n 7. Create MQTT Certificate\n ------- \033[0m")

    def create_custom_topic_and_mapping(self):
        if self.clientId == "":
            self.print_error("Please enter client id !")
            return
        ...
        try:
            ...
            self.print_error("Error finding aspect and variable 'moisture' for mapping")
            return

        if not found:
            print("Error finding aspect and variable 'moisture' for mapping")
            self.print_error("No aspect with variable 'moisture' for mapping")
            return
        ...
        self.print_success(serialized_response)
    ...
    def check_command_status(self):
        ...
        if self.job_id == "":
            self.print_error("Please fire a command first !")
            return
    ...
    def start_read_loop(self):
        while 1 == 1:
            ...
            if operation == "1":
                ...
                print("Create Auto Generated Certificate")
                self.submit_generate_agent_certificate()
                print("Finished creating auto generated certificate.")
